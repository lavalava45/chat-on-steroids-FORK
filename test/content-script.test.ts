...
describe('per-message Core attachment', () => {
  ...
  it('does not treat Chrome trusted input emitted by its own native mention insertion as user interruption', async () => {
    let sends = 0;
    live = await harness(undefined, { activity: () => activity }, document => {
      document.querySelector('[data-testid="send-button"]')!.addEventListener('click', () => { sends++; });
    });
    ...
    form.addEventListener = ((type: string, listener: EventListener, options?: boolean | AddEventListenerOptions) => {
      listeners.set(type, listener);
      add(type, listener, options);
    }) as typeof form.addEventListener;

    api.selectConnectorMention = async (_name: string, _id: string, stillCurrent: () => boolean) => {
      // Real Chrome reports the input generated by execCommand('insertText') as trusted.
      // A regression that listens to input will therefore cancel itself here.
      listeners.get('input')?.call(form, { isTrusted: true } as Event);
      selected = true;
      return stillCurrent();
    };
    live.document.querySelector('#prompt-textarea')!.textContent = 'continue using Core on this task';
  ...
  it('still fails closed when a real trusted editor change begins during Core attachment', async () => {
    let sends = 0;
    live = await harness(undefined, { activity: () => activity }, document => {
      document.querySelector('[data-testid="send-button"]')!.addEventListener('click', () => { sends++; });
    });
    ...
    api.selectConnectorMention = async (_name: string, _id: string, stillCurrent: () => boolean) => {
      listeners.get('beforeinput')?.call(form, { isTrusted: true } as Event);
      selected = true;
      return stillCurrent();
    };
    const composer = live.document.querySelector('#prompt-textarea')!;
