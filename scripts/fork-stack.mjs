...
function printStatus() {
  ...
  console.log(target ? `target: ${short(target)} (${target})` : 'target: <explicit release ref required for sync>');
...
function sync() {
  ...
  if (!target) {
    throw new Error(
      'Choose an explicit upstream release/tag/commit, for example: npm run fork:sync-upstream -- v2.1.15'
    );
  }
  if (readSyncState()) {
...
try {
  ...
  else throw new Error('Usage: node scripts/fork-stack.mjs <status|verify|sync|abort-sync> [explicit-release-ref]');
} catch (error) {
