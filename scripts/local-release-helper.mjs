...
const [targetExe, statePath, targetSlot, branch, commit] = process.argv.slice(2);
if (!targetExe || !statePath || !targetSlot || !branch || !commit) {
  process.exit(2);
}

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

function runningCosProcesses() {
...
while (Date.now() < deadline) {
  await sleep(500);
  if (runningCosProcesses().length === 0) break;
}

if (runningCosProcesses().length !== 0) process.exit(3);
